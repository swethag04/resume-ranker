...
def extract_job_req(job_desc):
    ...
    prompt_template = """
            ...
            Extract the key qualifications required for this role from the job description in JSON format.
            The keys in the JSON are job title, experience, technical skills, social skills,degree.
...
def match_resumes(pdf_folder, job_desc_json):
    # Define prompt
        ...
        Your role involves a meticulous process of evaluating resumes against specific job requirements.

        You are provided with resumes of multiple candidates  {text}

        You are also provided with the description for the job being hired {job_desc_json}

        Review the job description noting job title, experience, technical skills, social skills, degree required for the job.
        Each category is scored on a scale of 1 to 5, where 1 indicates a poor match and 5 indicates an excellent match.

        Before assigning scores, take a moment to reflect on the candidate's overall profile in relation to the job description.
        Evaluate the candidate's work history for relevance to the job's experience requirements, considering past roles, 
        industries, and levels of responsibility.
